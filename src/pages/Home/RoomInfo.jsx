...
const RoomInfo = ({ removeComponent, room }) => {
	...
	const handleRemoveComponent = () => {
		const totalDuration =
			innerContainerAnimationDuration + mainContainerAnimationDuration + 500;
		setInnerContainerStyle(innerContainerInitialState);
		setTimeout(
			() => setMainContainerStyle(mainContainerInitialState),
			innerContainerAnimationDuration
		);
	...
	useEffect(() => {
		roomsAPI
			.getRoomMembers(room.room_id, token)
			.then(({ data }) => {
	...
	return (
		<section id='room-info' style={mainContainerStyle}>
							...
							roomMembers.map(user => {
								return (
									<article key={user.user_id} className='user'>
										<img
											src={user.profile_picture || noProfile}
											alt=''
										...
										<div className='actions'>
											<Link
												...
												to={`${routes.userProfile.path}/${user.user_id}`}
											>
												see profile
											</Link>
										</div>
