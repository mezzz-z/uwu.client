...
const Rooms = ({ setShowCreateRoomModal }) => {
	...
	const joinRoom = async roomId => {
		...
		try {
			const {
				data: { room },
			} = await roomsAPI.getRoom(roomId, auth.token);
		...
		<>
			<section className='rooms-container'>
				<button
					...
					className='create-room-btn card not-button'
				>
					<svg
						width='28'
						height='28'
					...
					roomsState.rooms.map(room => {
						return (
							<div
								...
								key={room.room_id}
								className={`room card ${
									currentRoom.room_id === room.room_id && "current-room"
								}`}
							>
