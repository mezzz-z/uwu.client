...
const Friends = ({ setShowAddUserToRoomModal }) => {
	...
	const {
		...
		setAwaitFriends,
		updateFriendStatus,
		removeFriend: handleRemovingFriend,
		userState: { friends, awaitFriends },
	} = useUser();
	...
	const removeFriend = friendId => {
		socket.emit("friends/remove-friend", {
			friendId: friendId,
	...
	useEffect(() => {
		...
		socket.on(
			"friends/incoming-friend-request-answer",
			({ accepted, answerSender }) => {
				if (!accepted) return; // you can do more thing in future
				addNewFriend(answerSender);
			}
		);
		socket.on("friends/remove-friend-response", ({ success, friendId }) => {
			if (!success) return;
			handleRemovingFriend(friendId);
		});
		socket.on("friends/remove-friend-notification", ({ from: userId }) => {
			handleRemovingFriend(userId);
		});

		return () => {
			socket.removeListener("global/friend-status-changed");
			socket.removeListener("friends/incoming-friend-request-answer");
			socket.removeListener("friends/remove-friend-notification");
			socket.removeListener("friends/remove-friend-response");
		};
			...
			<div className='header'>
				...
				<div className='friends-count-container'>
					...
					<span className='count'>
						{awaitFriends ? "Loading..." : friends.length}
					</span>
				</div>
			</div>
							...
							<div key={friend.user_id} className='friend'>
								...
								<section className='profile-details'>
									...
									<div className='status-container'>
										<span>status:</span>
										<span className={`status status-${friend.status}`}>
											{friend.status}
										</span>
									</div>
								</section>
								<section className='actions-container'>
									...
									>
										...
										>
											<path
												d='M18 0H2C0.897 0 0 0.897 0 2V20L4 16H18C19.103 16 20 15.103 20 14V2C20 0.897 19.103 0 18 0ZM15 9H11V13H9V9H5V7H9V3H11V7H15V9Z'
												fill='#0D66EC'
											/>
										</svg>
									</button>
									<button
										...
										onClick={() =>
											handleVideoCallInvitation(friend.user_id, friend.status)
										}
									>
										<svg
											width='20'
